...
const int BERGCO_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString BERGCO_IPC_PREFIX("bergco:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(BERGCO_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(BERGCO_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(BERGCO_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click bergco: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start bergco: click-to-pay handler");
    else
...
{
    // clicking on bergco: URLs creates FileOpen events on the Mac:
