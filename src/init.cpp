...
{
#ifdef QT_GUI
    // ensure we leave the Qt main loop for a clean GUI exit (Shutdown() is called in bergco.cpp afterwards)
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: bergco.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: bergcod.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 12088 or testnet: 22088)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 12089 or testnet: 22089)") + "\n";
    ...
    strUsage += "\n" + _("SSL options: (see the Bergco Wiki for SSL setup instructions)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizebergcoamount=<n> " + _("Keep N Bergco anonymized (default: 0)") + "\n";
...
/** Sanity checks
 *  Ensure that Bergco is running in a usable environment with all
...
bool InitSanityCheck(void)
{
    if(!ECC_InitSanityCheck()) {
        InitError("OpenSSL appears to lack support for elliptic curve cryptography. For more "
                  "information, visit https://en.bergco.it/wiki/OpenSSL_and_EC_Libraries");
        return false;
    }
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Bergco is shutting down."));
...
#endif
    // Make sure only a single Bergco process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Bergco is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Bergco version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Bergco server starting\n");
    ...
    // requested to kill bergco-qt during the last operation. If so, exit.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Bergco") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Bergco to complete") << "\n";
    ...
    nAnonymizeBergcoAmount = GetArg("-anonymizebergcoamount", 0);
    if(nAnonymizeBergcoAmount > 999999) nAnonymizeBergcoAmount = 999999;
    if(nAnonymizeBergcoAmount < 2) nAnonymizeBergcoAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeBergcoAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize Bergco Amount %d\n", nAnonymizeBergcoAmount);
