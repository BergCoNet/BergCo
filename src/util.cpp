...
int nAnonymizeBergcoAmount = 100000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bergco
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bergco
    // Mac: ~/Library/Application Support/Bergco
    // Unix: ~/.Bergco
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bergco";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bergco.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No bergco.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override bergco.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bergcod.pid"));
...
// Format the subversion field according to BIP 14 spec (https://en.bergco.it/wiki/BIP_0014)
