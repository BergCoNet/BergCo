...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #bergcocoinTEST\r");
            Send(hSocket, "WHO #bergcocoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #bergcocoin\n");
            // randomly join #bergcocoin00-#bergcocoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #bergcocoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #bergcocoin%02d\r", channel_number).c_str());
        };
